...
router.post("/newpost", async (req, res) => {
    try {
        ...
        if (username !== "") {
            const isUserExist = await User.findOne({ username: username }, { new: true }).lean().exec();
            if (isUserExist) {
        ...
        if (hashTag !== "") {
            const isHashTag = await HashTag.findOne({ hashTagName: hashTag }).lean().exec();
            if (isHashTag) {
...
router.patch("/likepost/:id", async (req, res) => {
    try {
        ...
        await User.findByIdAndUpdate(isPost.userId, { isNewNotifications: true }, { new: true });
        return res.status(201).send(post)
    }
    catch (error) {
...
router.patch("/addcomment/:id", async (req, res) => {
    try {
        ...
        if (userId.toString() === post._id.toString()) {
            ...
            return res.status(201).send({ message: "you comment on your post", isUser })
        }
        ...
        await User.findByIdAndUpdate(userId, { isNewNotification: true }, { new: true });
        return res.status(201).send(post);
...
router.patch("/likecomment/:id", async (req, res) => {
    try {
        ...
        if (!isPost)
            return res.status(402).send({ error: "post is not exist" });
...
router.patch("/unlikecomment/:id", async (req, res) => {
    try {
        ...
        await Post.updateOne({ _id: postId }, { $set: { comments: comments } }, { new: true });
...
router.get("/user/:id", async (req, res) => {
    try {
        ...
        const userPost = await Post.find({ userId: userId }).lean().exec();
